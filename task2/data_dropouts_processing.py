"""
Выбросы - очень важная часть в предобработке данных.
Выбросы могут оказать нежелательное влияние на модель и 
подпортить предсказание.

Другая отдельная вещь - понять причину возникновения выброса,
является ли это случайностью и почему, какие мотивы.

В данном пакете приложения мы не рассматриваем семантические стороны выбросов,
а только рассматриваем их численные характеристики - проводим анализ и отбор значений.
"""
import pandas as pd
import numpy as np
